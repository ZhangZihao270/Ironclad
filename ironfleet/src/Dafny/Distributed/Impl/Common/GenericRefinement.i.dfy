...
module GenericRefinement_i {
  ...
  // function {:opaque} SetToSeq<T(!new)>(s : set<T>) : seq<T>
  // {
  //   seq(s)
  // }
  ...
  // lemma lemma_IsBijective<T(!new), CT(!new)>(s: set<CT>, ss: set<T>, f:CT~>T) returns (result:bool)
  //   requires forall x :: x in s ==> f.requires(x)
  //   requires forall x, y :: x in s && y in s && f(x) == f(y) ==> x == y
  //   requires forall x :: x in s ==> f(x) in ss
  //   requires forall y :: y in ss ==> exists x :: x in s && y == f(x)
  ...
  //   var mappedSet := set i | i in s :: f(i);
  //   if mappedSet != ss {
  //     result := false;
  //   }

  //   // Check if the function is injective (one-to-one)
  //   var inverseMap := map j | j in ss :: ( unique i: CT | i in s1 && f(i) == j :: i);
  //   if inverseMap.Values != set1 {
  //     result := false;
  //   }
  // }
