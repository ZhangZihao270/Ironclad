...
module LiveRSL__ReplicaImplClass_i {
  ...
  constructor()
  {
    ...
    // var rcs := CReplicaConstants(0, CConstants(CConfiguration([]), CParameters(0, 0, 0, 0, 0, 0)));
    ...
    // var acceptor_state :=
    //   AcceptorState(rcs, CBallot(0, 0), CVotes(map []), [], 0);
    // // var ep := EndPoint([]);
    // var learner_state := CLearnerState(rcs, CBallot(0, 0), map []);
    // // var app_state := AppStateMachine.Initialize();
    // var app_state := CAppState_Init();
    // var executor_state := ExecutorState(rcs, app_state, 0, CBallot(0, 0), COutstandingOpUnknown(), map[]);
