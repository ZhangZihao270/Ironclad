...
method{:timeLimitMultiplier 2} Receive(udpClient:UdpClient, localAddr:EndPoint, config:CConfiguration, msg_grammar:G) returns (rr:ReceiveResult, ghost udpEvent:UdpEvent)
            ...
            && SendLogReflectsBroadcast(udpEventLog, broadcast)
{
  ...
  if broadcast.CBroadcastNop? {
    ...
    // var time := Time.GetDebugTimeTicks();
    // if broadcast.msg.CMessage_2a? {
    //   if |broadcast.msg.val_2a| > 0 {
    //     var req := broadcast.msg.val_2a[0];
    //     print "req ", req.seqno, " send 2a at ", time, "\n";
    //   }
    // }
  }
}
