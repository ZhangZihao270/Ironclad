...
module Main_i refines Main_s {
  ...
  {
    && CConstantsIsAbstractable(ds.config)
  ...
  {
    RslState(AbstractifyCConstantsToLConstants(ds.config),
  ...
  lemma {:timeLimitMultiplier 2} lemma_DsConstantsAllConsistent(config:ConcreteConfiguration, db:seq<DS_State>, i:int, id:EndPoint)
    ...
    ensures  db[i].config == config
    ensures  db[i].servers[id].sched.replica.constants.all == AbstractifyCConstantsToLConstants(config)
    ensures  db[i].servers[id].sched.replica.proposer.constants.all == AbstractifyCConstantsToLConstants(config)
    ensures  db[i].servers[id].sched.replica.acceptor.constants.all == AbstractifyCConstantsToLConstants(config)
    ensures  db[i].servers[id].sched.replica.learner.constants.all == AbstractifyCConstantsToLConstants(config)
    ensures  db[i].servers[id].sched.replica.executor.constants.all == AbstractifyCConstantsToLConstants(config)
  {
    if i == 0
    {
  ...
  lemma lemma_GetImplBehaviorRefinement(config:ConcreteConfiguration, db:seq<DS_State>)
    ...
    ensures protocol_behavior[0].constants == AbstractifyCConstantsToLConstants(config)
  ...
  {
    c := AbstractifyCConstantsToLConstants(config);
    if |db| == 1 {
      ...
      calc {
        ...
        |AbstractifyCConstantsToLConstants(db[0].config).config.replica_ids|;
        |ls.constants.config.replica_ids|;
      }

      calc {
        ls.constants;
        AbstractifyCConstantsToLConstants(db[0].config);
        AbstractifyCConstantsToLConstants(config);
        c;
      }
