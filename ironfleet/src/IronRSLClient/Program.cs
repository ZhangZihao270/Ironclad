...
");
        ...
        static void Main(string[] args)
        {
            ...
            // IPAddress ip1 = IPAddress.Parse("130.245.173.102");
            // IPAddress ip2 = IPAddress.Parse("130.245.173.102");
            // IPAddress ip3 = IPAddress.Parse("130.245.173.102");
            IPAddress client_ip = IPAddress.Parse("127.0.0.1");
            IPAddress ip1 = IPAddress.Parse("130.245.173.102");
            IPAddress ip2 = IPAddress.Parse("130.245.173.103");
            ...
            var threads = ClientBase.StartThreads<Client>(num_threads, client_port, initial_seq_no, num_req_cnt_a, latency_cnt_ms_a).ToArray();
            ...
            else
            {
                ...
                for (int i = 0; i < num_threads; ++i)
                {
                    ...
                    Console.WriteLine("CLient {0} throughput {1} | avg latency ms {2}", i, num_req_cnt, latency_cnt_ms/num_req_cnt);
                }

                Console.WriteLine("throughput {0} | avg latency ms {1}", total_num_req_cnt / (int) (experiment_duration-8), total_latency_cnt_ms / total_num_req_cnt);
